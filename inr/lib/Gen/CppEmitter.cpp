...
// static inline bool emitTIRIn(
//     raw_stream& os, const Record* in, const Record* instRec,
//     const std::unordered_map<const Record*, std::string>& instMap) {
//     auto it = instMap.find(DefInit::get(in->getField("Inst")));
//     if(it == instMap.end()) return true;

//     sview inst = it->second;

//     it = instMap.find(instRec);
//     if(it == instMap.end()) return true;

//     sview instName = it->second;

//     const Type* type = IRTypeInit::get(in->getField("Type"));

//     const std::vector<const Init*>& operands =
//         ListInit::get(in->getField("Operands"));

//     unsigned indent = 4;

//     os.indent(indent) << "if(inst.getInstID() == " << inst << ") {\n";

//     // Make void "to ignore" type.
//     if(!type->isVoid()) {
//         indent += 4;
//         os.indent(indent) << "const Type* type = inst.getType();\n";
//         os.indent(indent) << "if(";
//         typeCondition(os, type);
//         os << ") {\n";

//         unsigned opNum = 0;

//         // TIRReg TIRImm TIRMem
//         for(const Init* opInit : operands) {
//             indent += 4;
//             const Record* tirOp = DefInit::get(opInit);

//             os.indent(indent) << "const TIROperand* op" << opNum
//                               << " = inst.getOperand(" << opNum << ");\n";
//             os.indent(indent) << "if(op" << opNum << "->";
//             if(tirOp->getName() == "TIRReg") {
//                 os << "isReg()";
//             }
//             else if(tirOp->getName() == "TIRMem") {
//                 os << "isMem()";
//             }
//             else if(tirOp->getName() == "TIRImm") {
//                 os << "isImm()";
//             }
//             os << ") {\n";
...
//     indent += 4;
//     os.indent(indent) << "MachineInst::create((uint32_t)" << instName
//                       << ", block, ";
//     if(operands.size()) {
//         os << "{\n";
//         indent += 4;
...
//         unsigned opNum = 0;
//         for(const Init* opInit : operands) {
//             if(opInit != operands.front()) {
//                 os << ",\n";
//             }
//             const Record* tirOp = DefInit::get(opInit);

//             os.indent(indent + 4) << "MachineOperand::";
//             if(tirOp->getName() == "TIRReg") {
//                 os << "createReg(op" << opNum << "->getReg())";
//             }
//             else if(tirOp->getName() == "TIRMem") {
//                 os << "createFrameIndex(op" << opNum << "->getMem())";
//             }
//             else if(tirOp->getName() == "TIRImm") {
//                 os << "createImm(op" << opNum << "->getImm())";
//             }
//             opNum++;
//         }
//         os << '\n';
